...
class User():
    ...
    __password = None  # Private class variable to store the hashed password
    ...
    def password(self, pwd):
        ...
        if pwd is None or type(pwd) is not str:
            self.__password = None  # Set password to None if not a valid string
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # Hash and store the password
    ...
    def is_valid_password(self, pwd):
        ...
        """
        if pwd is None or type(pwd) is not str:
            return False  # Invalid password if not a valid string
        if self.__password is None:
            return False  # Invalid password if no stored password
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # Compare hashed passwords
